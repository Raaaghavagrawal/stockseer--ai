...
st.markdown("""
    ...
    * {
    ...
    :root {
        ...
        --gradient-card: linear-gradient(145deg, #1e2332, #1a1f2e);
        --gradient-card-hover: linear-gradient(145deg, #252b3d, #1e2332);
        --gradient-button: linear-gradient(135deg, #00d4ff, #0099cc);
        --gradient-button-hover: linear-gradient(135deg, #66e6ff, #00d4ff);
    }
...
with st.sidebar:
    ...
    st.markdown("""
        <div style="text-align: center; padding: 20px 0;">
            <h1 style="font-size: 2rem; margin-bottom: 10px; background: linear-gradient(135deg, #00d4ff, #0099cc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
                📈 StockSeer.AI
            </h1>
    ...
    # Market Search and Filter Section
    st.markdown("""
        <div class="search-section">
            <h3 style="color: #00d4ff; text-align: center; margin-bottom: 20px;">
                🌍 Market Selection
            </h3>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div style="margin-bottom: 20px;">
            <label style="color: #b8c5d6; font-size: 0.9rem; font-weight: 600; margin-bottom: 8px; display: block;">
                🔍 Search Markets
            </label>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div style="margin: 20px 0;">
            <label style="color: #b8c5d6; font-size: 0.9rem; font-weight: 600; margin-bottom: 12px; display: block;">
                🌐 Filter by Region
            </label>
        </div>
    """, unsafe_allow_html=True)
    
    # Create a more compact region filter
    region_options = ["All Regions"] + list(market_regions.keys())
    ...
    if filtered_markets:
        ...
        # Display markets in a more compact format
        for market in filtered_markets[:8]:  # Limit to 8 markets for better UX
            info = MARKET_CONFIGS[market]
            market_icon = market_icons.get(market, "🌐")
            currency = info.get("currency", "")
            exchange = info.get("exchange", "")
            
            # Create a modern market card
            col1, col2, col3 = st.columns([3, 2, 1])
            
            with col1:
                st.markdown(f"""
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <span style="font-size: 1.5rem;">{market_icon}</span>
                        <div>
                            <div style="font-weight: 600; color: #ffffff;">{market}</div>
                            <div style="font-size: 0.8rem; color: #8a9bb8;">{exchange}</div>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                st.markdown(f"""
                    <div style="text-align: center; padding: 8px; background: rgba(0, 212, 255, 0.1); border-radius: 8px; border: 1px solid rgba(0, 212, 255, 0.2);">
                        <div style="font-weight: 600; color: #00d4ff;">{currency}</div>
                    </div>
                """, unsafe_allow_html=True)
            
            with col3:
                if st.button("Select", key=f"select_{market}", use_container_width=True):
                    st.session_state.selected_market = market
                    st.rerun()
    else:
        st.markdown("""
            <div style="text-align: center; padding: 20px; color: #8a9bb8;">
                <div style="font-size: 2rem; margin-bottom: 10px;">🔍</div>
                <div>No markets found matching your search</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    ticker = st.text_input(
        ...
        default_ticker,
        key="ticker_input",
        placeholder=f"e.g., {default_ticker}"
    ).upper()
    ...
    # Enhanced compare ticker input
    st.markdown("""
        <div style="margin: 20px 0;">
            <label style="color: #b8c5d6; font-size: 0.9rem; font-weight: 600; margin-bottom: 8px; display: block;">
                ⚖️ Compare Stock (Optional)
            </label>
        </div>
    """, unsafe_allow_html=True)
    ...
    # Create a more compact period selector
    selected_label = st.selectbox(
        "",
        list(period_options.keys()),
        index=list(period_options.keys()).index(st.session_state.selected_period_label),
        key="period_selector",
        label_visibility="collapsed"
    )
    
    if selected_label != st.session_state.selected_period_label:
        st.session_state.selected_period_label = selected_label
        st.rerun()
...
# --- MAIN APP LOGIC ---
if ticker:
    # Enhanced loading experience with explicit control
    loading_placeholder = st.empty()
    loading_placeholder.markdown("""
        <div class="loading-container">
            <div class="loading-spinner"></div>
            <div style="margin-left: 15px; color: #b8c5d6;">
                <div style="font-weight: 600;">Loading Stock Data...</div>
    ...
    try:
        ...
        # Clear loading message after successful data fetch
        loading_placeholder.empty()
        
        if not stock_info_main or stock_info_main.get('regularMarketPrice') is None:
            st.markdown("""
                <div style="text-align: center; padding: 40px; background: rgba(255, 71, 87, 0.1); border-radius: 16px; border: 1px solid rgba(255, 71, 87, 0.3);">
                    <div style="font-size: 3rem; margin-bottom: 20px;">⚠️</div>
                    <h3 style="color: #ff4757; margin-bottom: 15px;">Data Unavailable</h3>
                    <p style="color: #b8c5d6;">Essential data for this ticker is currently unavailable. Please check the ticker symbol or try again later.</p>
                </div>
            """, unsafe_allow_html=True)
            st.stop()
    except Exception as e:
        ...
        if any(keyword in error_str for keyword in ["could not resolve host", "failed to connect", "curl: (6)", "temporary issue"]):
            st.markdown("""
                <div style="text-align: center; padding: 40px; background: rgba(255, 170, 0, 0.1); border-radius: 16px; border: 1px solid rgba(255, 170, 0, 0.3);">
                    <div style="font-size: 3rem; margin-bottom: 20px;">🌐</div>
                    <h3 style="color: #ffaa00; margin-bottom: 15px;">Network Error</h3>
                    <p style="color: #b8c5d6;">Could not connect to Yahoo Finance. This is often a temporary issue.</p>
                    <p style="color: #8a9bb8; font-size: 0.9rem; margin-top: 10px;">Please check your internet connection and try again.</p>
                </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
                <div style="text-align: center; padding: 40px; background: rgba(255, 71, 87, 0.1); border-radius: 16px; border: 1px solid rgba(255, 71, 87, 0.3);">
                    <div style="font-size: 3rem; margin-bottom: 20px;">❌</div>
                    <h3 style="color: #ff4757; margin-bottom: 15px;">Error Loading Data</h3>
                    <p style="color: #b8c5d6;">An unexpected error occurred while loading data for {ticker}.</p>
                </div>
            """, unsafe_allow_html=True)
        st.stop()
    ...
    try:
        ...
        status_placeholder.markdown("""
            <div style="text-align: center; padding: 20px; color: #b8c5d6;">
                <div style="font-size: 2rem; margin-bottom: 10px;">📊</div>
                <div style="font-weight: 600;">Fetching Stock Information...</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        status_placeholder.markdown("""
            <div style="text-align: center; padding: 20px; color: #b8c5d6;">
                <div style="font-size: 2rem; margin-bottom: 10px;">📈</div>
                <div style="font-weight: 600;">Loading Price Data...</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        status_placeholder.markdown("""
            <div style="text-align: center; padding: 20px; color: #b8c5d6;">
                <div style="font-size: 2rem; margin-bottom: 10px;">🔍</div>
                <div style="font-weight: 600;">Calculating Technical Indicators...</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        if df.empty:
            ...
            st.markdown(f"""
                <div style="text-align: center; padding: 40px; background: rgba(255, 71, 87, 0.1); border-radius: 16px; border: 1px solid rgba(255, 71, 87, 0.3);">
                    <div style="font-size: 3rem; margin-bottom: 20px;">📊</div>
                    <h3 style="color: #ff4757; margin-bottom: 15px;">No Data Available</h3>
                    <p style="color: #b8c5d6;">No data available for {ticker}. Please check the ticker symbol.</p>
                </div>
            """, unsafe_allow_html=True)
            st.stop()
        ...
        status_placeholder.markdown("""
            <div style="text-align: center; padding: 20px; color: #b8c5d6;">
                <div style="font-size: 2rem; margin-bottom: 10px;">📚</div>
                <div style="font-weight: 600;">Loading Fundamental Data...</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    # Enhanced main content display
    st.markdown("""
        <div style="text-align: center; margin: 30px 0;">
            <h1 style="background: linear-gradient(135deg, #00d4ff, #0099cc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-size: 2.5rem; margin-bottom: 10px;">
    ...
    company_name = stock_info_main.get('shortName', ticker)
    st.markdown(f"""
        <div class="modern-card" style="text-align: center; margin-bottom: 30px;">
            <div style="display: flex; align-items: center; justify-content: center; gap: 20px; margin-bottom: 20px;">
                <div style="font-size: 3rem;">📊</div>
                <div>
                    <h2 style="color: #00d4ff; margin: 0;">{company_name}</h2>
                    <p style="color: #8a9bb8; margin: 5px 0; font-size: 1.1rem;">{ticker}</p>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
